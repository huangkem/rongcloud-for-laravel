...
class Message {
    ...
    public function publishPrivate($fromUserId, $toUserId, $objectName, $content, $pushContent = '', $pushData = '', $count = '', $verifyBlacklist, $isPersisted, $isCounted, $isIncludeSender) {
        try {
            if(empty($fromUserId))
                throw new RongCloudException('Paramer "fromUserId" is required');

            if(empty($toUserId))
                throw new RongCloudException('Paramer "toUserId" is required');

            if(empty($objectName))
                throw new RongCloudException('Paramer "$objectName" is required');

            if(empty($content))
                throw new RongCloudException('Paramer "$content" is required');
            ...
            $ret = $this->SendRequest->curl('/message/private/publish.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
    ...
    public function publishTemplate($templateMessage) {
        try {
            ...
            $ret = $this->SendRequest->curl('/message/private/publish_template.' . $this->format, $params, 'json', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function PublishSystem($fromUserId, $toUserId, $objectName, $content, $pushContent = '', $pushData = '', $isPersisted, $isCounted) {
        try {
            ...
            $ret = $this->SendRequest->curl('/message/system/publish.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function publishSystemTemplate($templateMessage) {
        try {
            ...
            $ret = $this->SendRequest->curl('/message/system/publish_template.' . $this->format, $params, 'json', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function publishGroup($fromUserId, $toGroupId, $objectName, $content, $pushContent = '', $pushData = '', $isPersisted, $isCounted, $isIncludeSender) {
        try {
            ...
            if(empty($toGroupId))
                throw new RongCloudException('Paramer "toGroupId" is required');
            ...
            $ret = $this->SendRequest->curl('/message/group/publish.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function publishDiscussion($fromUserId, $toDiscussionId, $objectName, $content, $pushContent = '', $pushData = '', $isPersisted, $isCounted, $isIncludeSender) {
        try {
            ...
            if(empty($toDiscussionId))
                throw new RongCloudException('Paramer "toDiscussionId" is required');
            ...
            $ret = $this->SendRequest->curl('/message/discussion/publish.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function publishChatroom($fromUserId, $toChatroomId, $objectName, $content) {
        try {
            ...
            if(empty($toChatroomId))
                throw new RongCloudException('Paramer "toChatroomId" is required');
            ...
            $ret = $this->SendRequest->curl('/message/chatroom/publish.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function broadcast($fromUserId, $objectName, $content, $pushContent = '', $pushData = '', $os = '') {
        try {
            ...
            $ret = $this->SendRequest->curl('/message/broadcast.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function getHistory($date) {
        try {
            if(empty($date))
                throw new RongCloudException('Paramer "date" is required');
            ...
            $ret = $this->SendRequest->curl('/message/history.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
    ...
    public function deleteMessage($date) {
        try {
            ...
            $ret = $this->SendRequest->curl('/message/history/delete.' . $this->format, $params, 'urlencoded', 'im', 'POST');
            if(empty($ret))
                throw new RongCloudException('bad request');
            return $ret;
