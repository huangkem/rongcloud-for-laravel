...
class Group{
	...
	public function create($userId, $groupId, $groupName) {
    	try{
			...
			if (empty($groupId))
				throw new RongCloudException('Paramer "groupId" is required');

			if (empty($groupName))
				throw new RongCloudException('Paramer "groupName" is required');
    		...
    		$ret = $this->SendRequest->curl('/group/create.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
	...
	public function sync($userId, $groupInfo) {
    	try{
			...
			if (empty($groupInfo))
				throw new RongCloudException('Paramer "groupInfo" is required');
    		...
    		$ret = $this->SendRequest->curl('/group/sync.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function refresh($groupId, $groupName) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/refresh.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function join($userId, $groupId, $groupName) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/join.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function queryUser($groupId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/user/query.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function quit($userId, $groupId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/quit.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function addGagUser($userId, $groupId, $minute) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/user/gag/add.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function lisGagUser($groupId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/user/gag/list.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function rollBackGagUser($userId, $groupId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/user/gag/rollback.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function dismiss($userId, $groupId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/group/dismiss.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
