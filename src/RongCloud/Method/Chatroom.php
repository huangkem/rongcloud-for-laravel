...
class Chatroom{
	...
	public function create($chatRoomInfo) {
    	try{
			if (empty($chatRoomInfo))
				throw new RongCloudException('Paramer "chatRoomInfo" is required');
    		...
    		$ret = $this->SendRequest->curl('/chatroom/create.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
	...
	public function join($userId, $chatroomId) {
    	try{
			...
			if (empty($chatroomId))
				throw new RongCloudException('Paramer "chatroomId" is required');
    		...
    		$ret = $this->SendRequest->curl('/chatroom/join.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function query($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/query.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function queryUser($chatroomId, $count, $order) {
    	try{
			...
			if (empty($count))
				throw new RongCloudException('Paramer "count" is required');

			if (empty($order))
				throw new RongCloudException('Paramer "order" is required');
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/query.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function stopDistributionMessage($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/message/stopDistribution.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function resumeDistributionMessage($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/message/resumeDistribution.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function addGagUser($userId, $chatroomId, $minute) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/gag/add.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function ListGagUser($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/gag/list.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function rollbackGagUser($userId, $chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/gag/rollback.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function addBlockUser($userId, $chatroomId, $minute) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/block/add.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function getListBlockUser($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/block/list.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function rollbackBlockUser($userId, $chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/block/rollback.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function addPriority($objectName) {
    	try{
			if (empty($objectName))
				throw new RongCloudException('Paramer "objectName" is required');
    		...
    		$ret = $this->SendRequest->curl('/chatroom/message/priority/add.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function destroy($chatroomId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/destroy.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
	...
	public function addWhiteListUser($chatroomId, $userId) {
    	try{
    		...
    		$ret = $this->SendRequest->curl('/chatroom/user/whitelist/add.'.$this->format,$params,'urlencoded','im','POST');
    		if(empty($ret))
    			throw new RongCloudException('bad request');
    		return $ret;
